...
# Part 1 Solution
sum = 0
input_array.each do |inner_array|
  sum += inner_array.max - inner_array.min
end

puts "Day 2, Part 1 Solution: #{sum}"
...
def get_array_evenly_divide_result(ary)
  ary.each_with_index do |v, i|
    ary.length.times do |elem|
      if ary.index(ary[elem]) != i
        evenly_divide_num = evenly_divide?(v, ary[elem])
        if evenly_divide_num > 0
          return evenly_divide_num
        end
      end
    end
  end
...
input_array.each do |line|
  sum += get_array_evenly_divide_result(line)
end

puts "Day 2, Part 2 Solution: #{sum}"
