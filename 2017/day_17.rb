...
#
# Your puzzle input is 316.

def spinlock_insert(spinlock_steps, spinlock_value_array, insert_value)
  start_position = spinlock_value_array.index(insert_value - 1)
  insert_position = (start_position + spinlock_steps) % spinlock_value_array.length
  first_slice = spinlock_value_array.slice(0..insert_position)
  second_slice = spinlock_value_array.slice(insert_position + 1..spinlock_value_array.length - 1)
  return first_slice + [insert_value] + second_slice
...
insert_value = 1
2017.times do
  spinlock_array = spinlock_insert(spinlock_steps, spinlock_array, insert_value)
  insert_value += 1
end

puts "Day 17 Solution 1: #{spinlock_array[spinlock_array.index(2017) + 1]}"
...
#
# Your puzzle input is still 316.
#
