...
# Input Array Dup for Part 2
input_array_p2 = input_array.dup

position = 0
steps = 0

while position < input_array.length do
  steps += 1
  current_space_val = input_array[position]
  input_array[position] += 1
  position = current_space_val + position
end

puts "Day 5, Solution 1: #{steps}"
...
input_array = input_array_p2
position = 0
steps = 0

while position < input_array.length
  steps += 1
  current_space_val = input_array[position]
  input_array[position] += current_space_val > 2 ? -1 : 1
  position = current_space_val + position
end

puts "Day 5, Solution 2: #{steps}"
