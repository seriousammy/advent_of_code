...
def find_or_create_register(register, ary)
  ary.each do |a|
    return a if a.first == register
  end
  new_register = [register, 0]
  ary << new_register
  new_register
end

def operator_result(val_1, val_2, str_operator)
  case str_operator
  when ">"
    val_1 > val_2
  when "<"
...
register_arrays = Array.new
max = 0
input_array.each do |line|
  # Format of [['b'], ['inc'], ['5'], ['if'], ['a'], ['>'], ['1']]
  register = find_or_create_register(line[0], register_arrays)
  inc_or_dec = line[1]
  val_change = line[2].to_i
  comp_register = find_or_create_register(line[4], register_arrays)
  operator = line[5]
  comp_val = line[6]
  if operator_result(comp_register[1], comp_val.to_i, operator)
    register_arrays.each do |a|
      if a.first == register[0]
        case inc_or_dec
        when "inc"
          register[1] += val_change
        when "dec"
          register[1] -= val_change
        end
        max = register[1] if max < register[1]
      end
    end
  end
end

puts "Day 8, Solution 1: #{register_arrays.max_by {|x| x[1]}[1]}"
