...
# you come upon a tower of programs that have gotten themselves into a bit of
...
File.readlines(file_name).each do |line|
  input_array << line.scan(/\w+/)
end

all_elem_ary = Array.new
input_array.each do |ary|
  ary.delete_at(1)
  ary.each do |elem|
    all_elem_ary << elem
  end
end
bottom_node = all_elem_ary.group_by {|x| x}.select { |k, v| v.length == 1 }.keys.first

puts "Day 7, Solution 1: #{bottom_node}"
