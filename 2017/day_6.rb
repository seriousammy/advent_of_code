...
# blocks, so it is chosen for redistribution. Starting with the next bank (the
# fourth bank) and then continuing to the first bank, the second bank, and so
# on, the 7 blocks are spread out over the memory banks. The fourth, first, and
# second banks get two blocks each, and the third bank gets one back. The final
...
input_array = [14,0,15,12,11,11,3,5,1,6,8,4,9,1,8,4]

array_history = Array.new

steps = 0
while !array_history.include? input_array do
  # Store modified array in history
  array_history << input_array.dup
  # Get the max value index, and redistribute value, and zero it after.
  index = input_array.index(input_array.max)
  blocks_to_redist = input_array[index]
  input_array[index] = 0
  # Redistribute block
  while blocks_to_redist > 0
    index_redist = (index_redist + 1) % input_array.length
    input_array[index_redist] += 1
    blocks_to_redist -= 1
  end
  steps += 1
end
# Store last input in history for Part 2.
array_history << input_array

puts "Day 6, Solution 1: #{steps}"
...
array_history.each_with_index do |val, i|
  if val == array_history.last
    cycles = array_history.length - (i + 1)
    break
  end
end

puts "Day 6, Solution 2: #{cycles}"
