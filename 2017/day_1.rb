...
input_line_array = Array.new

# Parse input text file
File.readlines(file_name).each do |line|
  input_line_array << line.to_s.strip
end
input_array = input_line_array.first.split("").map(&:to_i)

sum = 0

input_array.each_with_index do |val, index|
  next_elem = (index + 1) % input_array.length
  if val == input_array[next_elem]
    sum += val
  end
end

puts "Day 1, Part 1 Solution: #{sum}"
...
input_array.each_with_index do |val ,index|
  next_elem = (index + (input_array.length/2)) % input_array.length
  if val == input_array[next_elem]
    sum += val
  end
end

puts "Day 1, Part 2 Solution: #{sum}"
