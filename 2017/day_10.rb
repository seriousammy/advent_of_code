...
def triple_array_zip(index_ary, val_ary, change_ary)
  index_ary.each do |i|
    change_ary[i] = val_ary[index_ary.index(i)]
  end
  change_ary
end

def wrapped_process(start_index, length, ary)
  val_ary = Array.new
  index_ary = Array.new
  while length > 0
    index = start_index % ary.length
    val_ary << ary[index]
    index_ary << index
    start_index += 1
    length -= 1
  end
  ary = triple_array_zip(index_ary, val_ary.reverse, ary)
end

circular_list = (0..255).to_a
input_lengths = [157,222,1,2,177,254,0,228,159,140,249,187,255,51,76,30]
skip_size = 0
index = 0
input_lengths.each do |length|
  circular_list = wrapped_process(index, length, circular_list)
  index = (index + skip_size + length) % circular_list.length
  skip_size += 1
end

puts "Day 10 Solution 1: #{circular_list[0] * circular_list[1]}"
